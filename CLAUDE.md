...
## Guiding principles (weigh these in EVERY design decision)

1. **UX-first is the overriding principle.** Optimize for the user's experience, not implementation simplicity. When a choice trades UX for less code, choose UX (and surface the cost). This outranks every other principle here.
2. **North star — every AI-generated artifact must be traceable to its source, across EVERY function.** This is the project's defining purpose and applies to all AI outputs uniformly: `paper_qa` answers cite the exact chunks they used (Citation Canvas resolves each `[chunk:N]` back to source); `paper_search`/`paper_suggest` candidates carry their resolution evidence; `library_stats` curated papers each carry a per-pick reason; **slides record the paper section(s) each page was written from in `deck_slides` page metadata** (not on the visible slide — for eval + the Sources panel); every agent step records the IDs it touched (the agent-flow record principle below). **Any new AI-content feature MUST carry its source grounding by design — never bolt it on later.** When designing any function, ask first: how does its output trace back to source?
3. **Deterministic structure over prompt-reliance.** A must-do step (draft, verify/compile) is a pipeline node/guard, NOT an electable agent tool — exposing a mandatory step as a tool lets the model skip it, and forces you to *prompt* to enforce an invariant, which is unreliable. The agent decides judgment calls (what to edit); the pipeline guarantees the invariants (it always runs). Prefer focused, **lean** single-purpose agents over one fat multi-target prompt (a bloated prompt performs worse). Prompting is the LAST resort for control flow.
...
| F6 — Slide narrative planning + draft/revise split + grounding traceback + theme | **F6.1 complete** (planning + PaperDigest/targeted-read gather rework + streaming); **F6.2 pending** (split draft from revise: deterministic draft stages + revise-only agent + per-slide source traceback); F6.3 (Sources panel) · F6.4 (theme) pending | [F6.1](docs/superpowers/plans/2026-06-12-paperhub-F6.1-slide-narrative-planning.md) · [F6.2](docs/superpowers/plans/2026-06-15-paperhub-F6.2-slide-draft-revise-split.md) |
...
- `docs/superpowers/specs/` — SRS (**v2.35.0 current**; shipped through **v2.35.0**)
