...
Plans A–G are shipped + merged; closed follow-ups live in the SRS Revision History. Genuinely open, out-of-scope-when-written items:

- **(B) `RejectionPill` reachability** — wired but only renders on a `tool_call` with `status="rejected"`. Plan E's memory-scope rejection now produces that status, so **verify the pill renders**; Plan H's MCP-permission rejections will exercise it for real.
- **(D) HTML exact-chunk highlight needs a `dom_id`** — a chunk whose sentinel landed in a math/dangerous span is skipped at ingest (`postprocess_sentinels` → `dom_id=None`) and falls back to text-search / heading scroll. Closing it is a backend sentinel-placement fix (`pipelines/sentinels.py`); the frontend already degrades gracefully.
- **(E) `library_stats` auto-attach is prose-only** — it answers with numbers + the SQL but doesn't surface result rows as attachable cards like `paper_search`. (v2.32 added a frontend SQL *card* that lifts the ```sql block out of the prose; the rows themselves are still not attachable.)
- **(E) Memory recall is all-active, not semantic** — `build_active_memory_block` returns every active memory (≤20); the FTS/semantic path (`build_memory_context_block`) is kept but unused. Revisit if a user's active set grows large.
- **(F) deck-command classifier may return `target_scope="page"` + `target_page=null`** (didn't extract a Chinese ordinal). Handled deterministically — `_select_rows` falls back to `current_view_page`, so "edit this page" still hits the on-screen slide; few-shot the ordinal extraction for reliable named-page edits.
- **(F) `edit_slides` preserves notes by `slide_index`** — if an edit changes the frame count (a split), surplus notes drop (regenerate). `edit_frame`'s prompt forbids splitting, so it's a latent edge.
- **(F) `current_view_page` only sent when the session has a deck** — backend default `0` is normalized to page 1 in edit-scope reads.
