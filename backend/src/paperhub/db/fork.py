...
async def _copy_deck(
    ...
    *,
    ...
    deck the branch never had (and re-running it routes to EDIT instead of a
    fresh generate). A deck-producing run stamps ``runs.deck_version_id``; copy
    only when some run STRICTLY BEFORE the fork point did so."""
    async with conn.execute(
    ...
    if not deck_existed_at_fork_point:
        return  # the deck was generated AT/AFTER the fork point — branch is deckless
