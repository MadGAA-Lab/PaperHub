...
def test_route_qa_goes_to_sl_qa(monkeypatch) -> None:
    ...
    state = {"report_papers": [{"id": 1}], "report_command": DeckCommand(action="qa", target_page=None, note_language=None)}
...
def test_route_unknown_action_never_edits(monkeypatch) -> None:
    ...
    assert rg._route_deck_command(
        {"report_papers": [{"id": 1}], "report_command": DeckCommand(action="edit_slides", target_page=None, note_language=None)}
    ) == "edit_slides"
    assert rg._route_deck_command(
        {"report_papers": [{"id": 1}], "report_command": DeckCommand(action="qa", target_page=None, note_language=None)}
    ) == "qa"
...
async def test_sl_qa_delegates_to_answer_callback(monkeypatch, tmp_path) -> None:
    ...
    async with open_db(tmp_path / "t.db") as conn:
        ...
        state = {"run_id": run_id, "branch": "", "session_id": 1,
                 ...
                 "report_command": DeckCommand(action="qa", target_page=None, note_language=None),
