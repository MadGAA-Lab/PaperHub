# Plan F6.2 — Slide Pipeline: Split Draft from Revise
...
> **NORTH STAR — per-slide source traceability.** This project's defining goal: every AI-generated academic artifact traces back to its source. For slides that means **each slide page records the paper section(s) it was written from**, in **page metadata** (the DB `deck_slides` row), NOT on the visible slide. The pipeline must attach, per slide, the `(paper_id, section_name, chunk_ids)` it referenced — so a reviewer (or an automated eval) can check each slide's claims against exactly the section it cites. This is Task 6 and is **required**, not optional.
...
## Task 6: Per-slide source-section grounding (NORTH STAR — required)

Record, per slide, the paper section(s) it was written from — in `deck_slides`
**page metadata** (the DB, NOT the visible slide). Enables per-slide eval
(check the slide against exactly the section it cites) and the F6.2 Sources panel.

**Files:**
- Modify: `backend/src/paperhub/db/migrate.py` (add `deck_slides.source_sections_json`, idempotent PRAGMA-guarded like the existing migrations)
- Modify: `backend/src/paperhub/models/slide_domain.py` (`OutlineSlide.source_sections`)
- Modify: `backend/src/paperhub/agents/sl_outline.py` (`_resolve_slide` populates per-slide `source_sections` from `cites_reads`)
- Modify: the `sl_emit` step in `backend/src/paperhub/agents/report_graph.py` (write `source_sections_json` per `deck_slides` row)
- Test: `backend/tests/agents/test_report_pipeline.py` (or the emit test)

- [ ] **Step 1: Failing test.** After a GENERATE run, each `deck_slides` row has `source_sections_json` listing the `(paper_id, section_name, chunk_ids)` that slide cited; a slide with no source (title/agenda) has `[]`.
- [ ] **Step 2: Run, verify fail.**
- [ ] **Step 3a:** Migration — add nullable `TEXT` column `source_sections_json` to `deck_slides` (PRAGMA `table_info` guard, like the other migrations).
- [ ] **Step 3b:** `OutlineSlide` gains `source_sections: list[SourceSection]` where `SourceSection = {paper_id: int, section_name: str, chunk_ids: list[int]}`. `sl_outline._resolve_slide` already resolves each `cites_reads` key → `(paper_id, section)` + `chunk_ids` (it builds `support_excerpts`/`grounding_chunk_ids` from them) — populate `source_sections` from the same resolution.
- [ ] **Step 3c:** `sl_emit` maps each emitted content frame to its outline slide (same order; skip the leading title frame) and writes `json.dumps([s.model_dump() for s in slide.source_sections])` into that `deck_slides` row's `source_sections_json`.
- [ ] **Step 4: Run, verify pass.**
- [ ] **Step 5: Commit** `feat(slides): record per-slide source sections in deck_slides metadata (traceability north star)`.

Note: the revise agent's `read_section` calls are already traced at the agent level; attributing an *ad-hoc* revise read to a specific frame is a future enhancement — the outline-assigned per-slide sources are the primary, deterministic grounding recorded here.

---

## Task 7 (OPTIONAL, separate concern): simplify Outline to digest-only
...
## Task 8: SRS update + live verification
...
- **Spec coverage:** draft/revise split (Tasks 1-4), DRY (5), per-slide source grounding / NORTH STAR (6, required), outline cleanup (7, optional), SRS+live (8). The 0-tool-calls failure is fixed by Task 1+3 (draft is deterministic; the tool can't be skipped). Verification-as-pipeline-guard (Task 3) fixes the compile_check/density_check-as-tools anti-pattern.
