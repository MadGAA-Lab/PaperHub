...
*"Find recent papers on Mixture-of-Experts routing."* → Router picks `paper_search` → Research Agent runs a **tool-calling loop** (v2.3): per turn it decides whether to (a) ask a clarifying question when the user intent is too vague to search (e.g. *"find me good ML papers"* → *"What problem are you trying to solve — routing, training stability, or something else?"*), (b) call `search_arxiv(query, max_results)` via the copied-and-adapted Python helper from `paper2slides-plus` with a focused query, or (c) call `search_arxiv` again with a refined query (capped at N=3 calls per turn) before finalising. The agent's final message is a ranked, annotated suggestion list — typically *"I'd add papers X, Y, Z because…"* — with the metadata rows rendered as a result list with abstract, authors, year, and an **Add as reference** button per row. **Nothing is downloaded yet** — search results are metadata only, presented for the user to skim. Multi-turn refinement across user replies is supported by the same agent reading prior `AgentState.history`; the user can answer the clarifying question in the next chat turn and the agent picks up the refined intent.
...
| **FR-07** | In-repo pipelines (paper + slide) | `paper_search` is a **tool-calling Research Agent loop** (v2.3) whose mission is to **extend the session's reference set** on the user's behalf, **informed by what's already in it** (most research builds on prior work: the agent must be able to see "this session already has Vaswani et al. 2017 — let me find follow-up MoE routing work that cites it"). Each invocation's prompt is constructed with a **current-references context block** — title, arxiv_id, year, and short abstract for every currently-enabled paper in the session — so the agent reasons about gaps and follow-ups before searching. Tool palette: `search_library(query, max_results)` (read — full-text search across the user's deduplicated `paper_content` rows that aren't yet attached to the current session; returns title / arxiv_id / abstract / `paper_content_id`), `search_arxiv(query, max_results)` (read, external, via the copied-and-adapted Python helper from `paper2slides-plus`), `find_related_papers(arxiv_id, mode: cites | cited_by | similar, max_results)` (read, external — Semantic Scholar REST helper for citation-graph navigation; the canonical tool for "find me follow-up work to X" / "what does this paper cite" / "find papers like this one"; returns title / arxiv_id (when available) / abstract), and `add_paper_to_session(arxiv_id_or_paper_content_id, reason)` (write — invokes the Paper Pipeline §III-5.1, which short-circuits on cache hit so attaching an already-indexed library paper costs ~one INSERT; the resulting `papers` row is inserted with `enabled=true`, the schema default, so the addition is **immediately effective** for the next `paper_qa` turn without any user gesture). **Canonical per-turn flow** (the agent prompt encodes this priority order):
...
| **Research Agent** | Handles `paper_search` as a **tool-calling loop** (v2.3) whose mission is to **extend the session's reference set**, informed by what's already in it (follow-up-paper discovery is a first-class workflow). Three tools: `search_library(query, max_results)` (read — search the user's deduplicated `paper_content` library, excluding papers already in this session), `search_arxiv(query, max_results)` (read, external), and `add_paper_to_session(arxiv_id_or_paper_content_id, reason)` (write — calls the Paper Pipeline §III-5.1, cache-aware so attaching a library paper is essentially free; inserts `papers` with `enabled=true` so the addition is immediately effective for the next `paper_qa` turn). The agent prefers library hits over arXiv when both fit, since adding from the library skips download / extraction / chunking / embedding entirely. Per invocation the agent reads `AgentState.history + user_message + current-references context block` (titles, arxiv_ids, abstracts of all currently-enabled session references) and decides whether to (a) emit a clarifying question and stop, (b) call `search_arxiv` once then default-add the top 1-2 results, or (c) call `search_arxiv` up to N=3 times with refined queries before default-adding. The agent does not propose papers already present in the context block. Each tool invocation is wrapped by the Tool-Call Tracer (FR-09). The final assistant message names which papers were auto-added (now enabled in the session) and which remaining results the user can attach manually via **Add as reference** buttons on the `SearchResultList` rows. **The agent never toggles `enabled`, never deletes `papers` rows, never mutates `paper_content` / `chunks` directly — see FR-07.** Toggle / remove / manual-attach-from-library are user gestures backed by REST endpoints, used to adjust what the agent extended. Cross-turn refinement is automatic — the next invocation sees the prior conversation and the current curated reference set. Also handles `paper_qa` (in-repo RAG over the session's enabled reference sources); surfaces retrieved chunk IDs so the Citation Canvas can resolve them. | Paper Pipeline (§III-5), Chroma, LLM (flagship) |
